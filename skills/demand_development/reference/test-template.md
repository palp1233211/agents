...
> ⚠️ 测试验证完成后，**必须将 3.13 的验收输出结构写入文件**，不能只输出在对话中。

```bash
# 文件路径（固定格式）
workspace/docs/{需求号}-测试用例.md
```

**内容要求**：按 3.13 验收输出结构填写，包含：
- 测试用例清单（用例数、覆盖功能点、测试文件路径）
- 测试结果汇总表
- 反伪通过自检 A/B/C/D/E 逐项勾选
- 验收结论

**自检**：提交 Step 3 通过前，确认该文件已通过 Write 工具创建，不能只输出在对话中。

---

## 每步验收确认语

> "以上测试是否通过？请回复「通过」进入 Step 4（Yapi 文档同步），或指出需要调整的地方。"